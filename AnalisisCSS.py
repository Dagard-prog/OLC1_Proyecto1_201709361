...
reservadas = ['color','background-color','background-image','border','Opacity','background',
'text-align','font-family','font-style','font-weight','font-size','font','padding-left',
'padding-right','padding-bottom','padding-top','padding','display','line-height','width','height',
'margin-top','margin-right','margin-bottom','margin-left','margin','border-style','display'
,'position','bottom','top','right','left','float','clear','max-width','min-width','max-height','min-height',
'px','em','vh','vw','in','cm','mm','pt','pc','url']

signos = {"PORCENTAJE":'%',"NUMERAL":'\#',"PUNTOCOMA":';', "LLAVEA":'{', "LLAVEC":'}', "PARA":'\(', "PARC":'\)', "IGUAL":'=',
          "MAYOR":'>', "MENOR":'<',
...
def CSS(text):
    ...
    while counter < len(text):
        ...
        if re.search(r"[A-Za-z]", text[counter]): #IDENTIFICADOR
            ...
            aux = LPTM(linea, columna, text, text[counter])
            if(aux != None):
                listaTokens.append(aux)
        else:
            #SIGNOS
